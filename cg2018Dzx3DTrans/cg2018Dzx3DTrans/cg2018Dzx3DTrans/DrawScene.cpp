...
CDrawScene::CDrawScene(void)
{
}

CDrawScene::CDrawScene(Ccg2018Dzx3DTransView* pView)
{
	m_pView = pView;
...
void CDrawScene::DrawScene()
{
	...
	DWORD opStart = GetTickCount();       //记录测试开始时间，单位是毫秒
	...
	for (m_objNumber = 0; m_objNumber < pDoc->m_objectNum; m_objNumber++) {  
		...
		if (pDoc->m_transSelect < 3 && m_objNumber == pDoc->m_transSelect) CaculateMatrix();
...
// 
	...
	DWORD opFinish = GetTickCount();      //记录测试结束时间，单位是毫秒

	pDoc->m_DrawSceneRunTime = opFinish - opStart / 1000.0;
...
{
	Ccg2018Dzx3DTransDoc *pDoc = (Ccg2018Dzx3DTransDoc *)m_pView->GetDocument();	
	float x = m_whoObject->objCenter.x;
	float y = m_whoObject->objCenter.y;
	float z = m_whoObject->objCenter.z;
...
void CDrawScene::pRemoveBackFace()
{
	...
	for (i = 0; i < m_whoObject->polyCount; i++) {
		...
		if (pv >  0) m_whoObject->objectSpace[i].polyVisible = true;
		else         m_whoObject->objectSpace[i].polyVisible = false;
	}
}
		...
		if (m_whoObject->objectSpace[k].polyVisible) {  // Polygon visible
			...
			for (i = 0; clip && i < 6; i++) {  // Clip Box 6 Pane Cube 
				...
				for (j = 1; j < inCount; j++) {
					...
					if (spCross) pLineInterSectPane(sx, sy, sz, px, py, pz, i, &outCount, 
						                            m_whoObject->objectSpace[k].clipObject);
					sx = px;
					sy = py;
					sz = pz;
					pv = pVisible(sx, sy, sz, i);
	                               ...
	                               int pane, int *outCount, Gpoint_p polyClip)
{
	...
	switch (pane) {
	case 0: // SP line intersect with Left winLpane
		...
		iz = (pz - sz) * u + sz;
		break;
	case 1: // SP line intersect with Right winRpane
		...
		iz = (pz - sz) * u + sz;
		break;
	case 2: // SP line intersect with Top winTpane
		...
		iz = (pz - sz) * u + sz;
		break;
	case 3: // SP line intersect with Bottom winBpane
		...
		iz = (pz - sz) * u + sz;
		break;
	case 4: // SP line intersect with Front zFront
		iz = pDoc->zFront;
		u = (pDoc->zFront - sz) / (pz - sz);
		ix = (px - sx) * u + sx;
		iy = (py - sy) * u + sy;
		break;
	case 5: // SP line intersect with Back xBack
		iz = pDoc->zBack;
